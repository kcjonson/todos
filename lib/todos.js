...
// Private 
			...
			function(database){
				database.collection('todos', function(error, todosCollection) {
					if (error) {
						...
						// Create Mongo Ids for Categories
						todoData.categories.forEach(function(categoryObject){
							categoryObject._id = new mongoId(categoryObject._id);
						})
	...
	function getCategories(categoryStubs, callback) {
		 console.log('getCategories: ', categoryStubs);
		 getDatabase(
			function(database){
				database.collection('categories', function(error, collection) {				
					if (error) {
						console.console.log('Collection error: ', error);
					} else {					
						var categories = [];
						var errors = [];
						categoryStubs.forEach(function(categoryStub){
							collection.findOne({_id: categoryStub._id}, function(error, document){
								if (!error) {
									categories.push(document);
								} else {
									errors.push(error);
								}
								if ((categories.length + errors.length) >= categoryStubs.length) {
									if (errors.length < 1) {
	...
	function createCategory(successCallback, failureCallback) {
		getDatabase(
			function(database){
				database.collection('categories', function(error, categoriesCollection) {
					if (error) {
						failureCallback(error);
					} else {					
						var categoryData = {
						...
						categoriesCollection.insert(categoryData, {safe: true}, function(error, result) {
				            if (error) {
				            	console.log('Errors in create category', error);
				            	//failureCallback(error);
				            } else {
						...
						todosIds.forEach(function(todoId){
							collection.findOne({_id: todoId}, function(error, document){
								if (!error) {
									...
									errors.push(error);
									//checkForCompletion();  // This breaks things :(
								}								
							})
						});							
					};
